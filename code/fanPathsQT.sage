...
R.<r,q,t,u1,u2,u3,u4,u5,u6,u7,u8,u9,u10,u11,u12,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12,w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,y1,y2,y3,y4,y5,y6,y7,y8,y9,y10,y11,y12> = QQ['r','q','t','u1','u2','u3','u4','u5','u6','u7','u8','u9','u10','u11','u12','v1','v2','v3','v4','v5','v6','v7','v8','v9','v10','v11','v12','w1','w2','w3','w4','w5','w6','w7','w8','w9','w10','w11','w12','x1','x2','x3','x4','x5','x6','x7','x8','x9','x10','x11','x12','y1','y2','y3','y4','y5','y6','y7','y8','y9','y10','y11','y12']
...
Sigma = {
    ...
    (0, 0, 0, 0): s[6] + s[4, 1] + s[3, 1] + s[1, 1, 1],
    (0, 0, 0, 0, 0): s[1, 1, 1, 1] + s[3, 1, 1] + s[4, 1, 1] + s[4, 2] + s[4, 3] + s[5, 1, 1] + s[6, 1] + s[6, 2] + s[7, 1] + s[8, 1] + s[10],
    (1, 0, 0, 0, 0): s[3, 3] + s[4, 1, 1] + s[5, 2] + s[6, 1] + s[7, 1] + s[9] + (-u3)*s[2, 2] + (-v3)*s[3, 2] + (-w3)*s[4,2] + (-x3)*s[5,2] + (-y3)*s[3,3], #1
    (2, 0, 0, 0, 0): s[3, 1, 1] + s[4, 2] + s[5, 1] + s[6, 1] + s[8] + u2*s[2, 2] + v2*s[3, 2] + w2*s[4,2] + x2*s[5,2] + y2*s[3,3], #2
    (1, 1, 0, 0, 0): s[3, 1, 1] + s[4, 2] + s[5, 1] + s[6, 1] + s[8] + u3*s[2, 2] + v3*s[3, 2] + w3*s[4,2] + x3*s[5,2] + y3*s[3,3], #3
    (3, 0, 0, 0, 0): s[2, 1, 1] + s[4, 1] + s[5, 1] + s[7] + (u3-u2)*s[2, 2] + (1+v3-v2)*s[3, 2] + (w3-w2)*s[4,2] + (x3-x2)*s[5,2] + (y3-y2)*s[3,3], #4
    (2, 1, 0, 0, 0): s[5, 1] + s[7] + 0*s[2, 2] + 1*s[3, 2] + 0*s[4,2] + 0*s[5,2] + 0*s[3,3], #5
    (1, 1, 1, 0, 0): s[2, 1, 1] + s[4, 1] + s[5, 1] + s[7] + 0*s[2, 2] + 1*s[3, 2] + 0*s[4,2] + 0*s[5,2] + 0*s[3,3], #6
    (4, 0, 0, 0, 0): s[1, 1, 1] + s[3, 1] + s[4, 1] + s[6],
    (3, 1, 0, 0, 0): s[4, 1] + s[6] + 0*s[2, 2] + 0*s[3, 2] + 0*s[4,2] + 0*s[5,2] + 0*s[3,3], #7
    (2, 2, 0, 0, 0): s[2, 1, 1] + 2*s[4, 1] + s[6] + (1-u3)*s[2, 2] + (-v3)*s[3, 2] + (-w3)*s[4,2] + (-x3)*s[5,2] + (-y3)*s[3,3], #8
    (2, 1, 1, 0, 0): s[4, 1] + s[6] + (1-u3)*s[2, 2] + (-v3)*s[3, 2] + (-w3)*s[4,2] + (-x3)*s[5,2] + (-y3)*s[3,3], #9
    (1, 1, 1, 1, 0): s[1, 1, 1] + s[3, 1] + s[4, 1] + s[6],
    (4, 1, 0, 0, 0): s[3, 1] + s[5],
    (3, 2, 0, 0, 0): s[3, 1] + s[5] + 0*s[2, 2] + 0*s[3, 2] + 0*s[4,2] + 0*s[5,2] + 0*s[3,3], #10
    (3, 1, 1, 0, 0): s[3, 1] + s[5] + 1*s[2, 2] + 0*s[3, 2] + 0*s[4,2] + 0*s[5,2] + 0*s[3,3], #11
    (2, 2, 1, 0, 0): s[3, 1] + s[5] + (u3-1)*s[2, 2] + v3*s[3, 2] + w3*s[4,2] + x3*s[5,2] + y3*s[3,3], #12
    (2, 1, 1, 1, 0): s[3, 1] + s[5],
    (4, 2, 0, 0, 0): s[2, 1] + s[4],
    (4, 1, 1, 0, 0): s[2, 1] + s[4],
    (3, 3, 0, 0, 0): s[1, 1, 1] + s[2, 1] + s[3, 1] + s[4],
...
sage: ((Sigma[(0,)*n]).coproduct() - add([tensor([Sigma[partitionPath(path)], Sigma[partitionPath(zeta(n,n,path))]]) for path in DyckWords(n)])).coefficients()
...
sage: (Sigma[(0,)*n]).coproduct() - add([tensor([Sigma[partitionPath(path)], Sigma[psi[partitionPath(path)]]]) for path in DyckWords(n)])
-s[2, 2] # s[3] + s[2, 2] # s[4] - s[3] # s[2, 2] + s[4] # s[2, 2]
